...
###### BEGIN YOUR CODE ######
v_func = np.random.uniform(low=0, high=0.1, size=NUM_STATES)
prob_mat = np.ones((NUM_STATES, NUM_ACTIONS, NUM_STATES)) / NUM_STATES
rewards = np.zeros(NUM_STATES)

tmp_rewards = np.zeros(NUM_STATES)
tmp_state_action_new_state = np.zeros((NUM_STATES, NUM_ACTIONS, NUM_STATES))
tmp_state_action = np.zeros((NUM_STATES, NUM_ACTIONS))
###### END YOUR CODE ######
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    # model.
    ###### BEGIN YOUR CODE ######
    action = np.argmax(prob_mat[state] @ v_func)
    ###### END YOUR CODE ######
    ...
    ###### BEGIN YOUR CODE ######
    tmp_state_action[state, action] += 1
    tmp_state_action_new_state[state, action, new_state] += 1
    tmp_rewards[new_state] += R
    ...
    if new_state == NUM_STATES - 1:
        ...
        ###### BEGIN YOUR CODE ######
        tmp_state_visits = np.sum(tmp_state_action_new_state, axis=(0, 1))
        tmp_visited_states = np.nonzero(tmp_state_visits)
        rewards[tmp_visited_states] = tmp_rewards[tmp_visited_states] / tmp_state_visits[tmp_visited_states]
        indices_to_update = np.nonzero(tmp_state_action)
        prob_mat[indices_to_update] = tmp_state_action_new_state[indices_to_update] / tmp_state_action[indices_to_update].reshape(-1, 1)
        ...
        while diff > TOLERANCE:
            v_prev = v_func
            v_func = rewards + np.max(prob_mat @ (GAMMA * v_func), axis=1)
            diff = np.max(np.abs(v_prev - v_func))
            i += 1
        if i == 1:
