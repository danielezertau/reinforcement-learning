        ...
        for state in range(mdp.nS):
            ...
            for action in range(mdp.nA):
                ...
                for (p, next_state, r) in mdp.P[state][action]:
                    action_reward += p * (r + GAMMA * Vprev[next_state])
...
Vs_VI = np.array(Vs_VI)
for state in range(mdp.nS):
    plt.plot(np.arange(21), Vs_VI[:, state], label=f"State {state}")
plt.legend(loc="center left", bbox_to_anchor=(1, 0.5))
plt.tight_layout()
# plt.show()
plt.savefig('value-iter.pdf')
